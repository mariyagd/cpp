...
Data::Data( std::string s_date, std::string & s_rate ) : _date(""), _rate(0.0f), _error_msg("") {
	...
	if (isValidDate(s_date) && isValidRate(s_rate) )
	{
		this->_date = s_date;
...
//	std::cout << "\"" << s_rate << "\"" << std::endl;
	...
	if (point > 1)
	{
		this->_error_msg = "Error: Invalid rate ( too many points ) =>" + s_rate;
		return false;
	}
...
std::string Data::getDate( void ) const {
...
std::string Data::getErrorMsg( void ) const {
